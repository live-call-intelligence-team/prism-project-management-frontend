...
const featureSchema = z.object({
    ...
    status: z.enum(['TO_DO', 'IN_PROGRESS', 'IN_REVIEW', 'CLOSED']).optional(),
});
...
interface EditFeatureModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (id: string, data: Partial<Feature>) => Promise<void>;
    projectId: string;
    feature: Feature | null;
}

export function EditFeatureModal({ isOpen, onClose, onSubmit, projectId, feature }: EditFeatureModalProps) {
    ...
    useEffect(() => {
        if (isOpen && feature) {
            ...
            setValue('epicId', feature.epicId || '');
            setValue('priority', feature.priority);
            setValue('storyPoints', feature.storyPoints || 0);

            // Format dates for input type="date"
            if (feature.startDate) {
                setValue('startDate', new Date(feature.startDate).toISOString().split('T')[0]);
            } else {
                setValue('startDate', '');
            }
            if (feature.endDate) {
                setValue('endDate', new Date(feature.endDate).toISOString().split('T')[0]);
            } else {
                setValue('endDate', '');
            }

            setValue('tags', feature.tags ? feature.tags.join(', ') : '');
            // We'll need to fetch the ownerId if mapped differently or handle it
            setValue('assignedTo', feature.ownerId || '');
            setValue('status', feature.status === 'IN_REVIEW' ? 'IN_REVIEW' : feature.status === 'IN_PROGRESS' ? 'IN_PROGRESS' : feature.status === 'CLOSED' ? 'CLOSED' : 'TO_DO');

            // If there's an acceptanceCriteria field from the backend
            // setValue('acceptanceCriteria', feature.acceptanceCriteria || '');
        } else if (!isOpen) {
            reset();
        }
    }, [isOpen, feature, setValue, reset]);
    ...
    const handleFormSubmit = async (data: FeatureFormData) => {
        ...
        try {
            ...
            // Handle clearing epic if set to "No Epic"
            if (formattedData.epicId === "") {
                formattedData.epicId = null;
            }

            await onSubmit(feature.id, formattedData);
            onClose();
        } catch (error) {
